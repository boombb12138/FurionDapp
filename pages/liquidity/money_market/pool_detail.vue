  ...
  <div class="!w-1150px">
    <div class="absolute pt-70px flex justify-between items-center w-200px">
      <img
        ...
        @click="$router.go(-1)"
      />
      <div @click="refreshData" class="refresh">
        <span class="font-500">Refresh Data</span>
      </div>
    </div>
    ...
    <div v-if="active === 1" class="w-600px mx-auto box py-28px px-15px">
      ...
      <div class="flex px-25px mb-27px">
        <div class="flex-1 text-14px leading-18px text-left">
          ...
          <div class="font-500 text-16px">
            <span class="font-600 text-[#FCFFFD]">{{
              displayFormat(user_info.borrow_quota, token_decimal)
            }}</span>
            {{ asset }}
          </div>
        </div>
        <div class="flex-1 text-14px leading-18px text-center">
          ...
          <div class="font-500 text-16px">
            <span class="font-600 text-[#FCFFFD]">{{
              displayFormat(user_info.borrowed, token_decimal)
            }}</span>
            {{ asset }}
          </div>
        </div>
        <div class="flex-1 text-14px leading-18px text-right">
          ...
          <div class="font-500 text-16px">
            <span class="font-600 text-[#FCFFFD]"
              >{{ displayFormat(market_info.borrow_rate) }}%</span
            >
          </div>
        </div>
      </div>
        ...
        <div class="flex justify-between items-end">
          <div class="flex items-end">
            <el-input
              ...
              type="number"
            ></el-input>
            <div class="text-15px mb-10px ml-1px text-[rgba(252,255,253,0.6)]">
              ~${{ displayFormat(approxValue(borrow_amount)) }}
            </div>
          </div>
          <div class="text-12px mb-14px text-[rgba(252,255,253,0.6)]">
            Balance: {{ displayFormat(user_info.token_balance, token_decimal) }}
          </div>
        </div>
      </div>
      ...
      <div class="btn_border">
        <!-- 如果  大于  或者 或者 大于 或者borrow_amount是空的 那么就禁用按钮 -->
        <el-button
          type="primary"
          class="!w-full !h-60px"
          :disabled="
            compareFormat(borrow_amount, token_decimal) >
              parseInt(user_info.borrow_quota) ||
            compareFormat(borrow_amount, token_decimal) >
              parseInt(market_info.cash) ||
            borrow_amount === ''
          "
    ...
    <div v-if="active === 2" class="w-600px mx-auto box py-28px px-15px">
      ...
      <div class="btn_border">
        <el-button
          ...
          class="!w-full !h-60px"
          :disabled="
            compareFormat(repay_amount, token_decimal) >
              parseInt(user_info.token_balance) || repay_amount === ''
          "
          @click="repay(repay_amount)"
        >
      ...
      <p class="reminder">
        Note: Toggle the
        <span class="text-[#f181de]">close position</span> switch to get the
        amount you need to repay to close your borrowing position.
      </p>
    </div>
    ...
    <div v-if="active === 3" class="w-600px mx-auto box py-28px px-15px">
      <div class="flex px-25px mb-27px">
        <div class="flex-1 text-14px leading-18px">
          ...
          <div class="font-500 text-16px">
            <span class="font-600 text-[#FCFFFD]">{{
              displayFormat(user_info.withdraw_quota, token_decimal)
            }}</span>
            {{ asset }}
          </div>
        </div>
        <div class="flex-1 text-14px leading-18px text-center">
          ...
          <div class="font-500 text-16px">
            <span class="font-600 text-[#FCFFFD]">{{
              displayFormat(user_info.deposited, token_decimal)
            }}</span>
            {{ asset }}
          </div>
        </div>
        <div class="flex-1 text-14px leading-18px text-right">
          ...
          <div class="font-500 text-16px">
            <span class="font-600 text-[#FCFFFD]"
              >{{ displayFormat(market_info.supply_rate) }}%</span
            >
          </div>
        </div>
      </div>
      ...
      <div class="btn_border">
        <el-button
          ...
          class="!w-full !h-60px"
          :disabled="
            compareFormat(deposit_amount, token_decimal) >
              parseInt(user_info.token_balance) || deposit_amount === ''
          "
    ...
    <div v-if="active === 4" class="w-600px mx-auto box py-28px px-15px">
      ...
      <div class="btn_border">
        <el-button
          ...
          class="!w-full !h-60px"
          :disabled="
            compareFormat(withdraw_amount, token_decimal) >
              parseInt(user_info.withdraw_quota) ||
            compareFormat(withdraw_amount, token_decimal) >
              parseInt(user_info.deposited) ||
            withdraw_amount === ''
          "
          @click="withdraw(withdraw_amount)"
        >
          <span class="font-800 text-20px" style="word-spacing: 5px"
            >WITHDRAW {{ asset }}</span
          >
      ...
      <p class="reminder">
        Note: Toggle the <span class="text-[#f181de]">withdraw max</span> switch
        to get the maximum amount you can withdraw without causing shortfall to
        your borrowings.
      </p>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    this.token = await initTokenContract(this.asset); //token就是initTokenContract返回的token_contract
    ...
    // 如果asset不是ETH，就将 token_decimal 赋值为..默认值为18
    if (!this.is_eth) {
  ...
  methods: {
    ...
    async updateUserInfo() {
      ...
      if (results[3]["1"] > 0) {
        ...
        this.user_info.withdraw_quota =
          tempLiquidity > parseInt(this.user_info.deposited)
    ...
    async writeRepayAmount() {
      if (this.close_position) {
        ...
        if (this.user_info.borrowed == 0) {
          ...
          this.repay_amount = parseFloat(
            fromWei(this.user_info.borrowed, this.token_decimal)
          ).toFixed(decimals);
        }
      } else {
        this.repay_amount = "";
    ...
    async writeWithdrawAmount() {
      if (this.withdraw_max) {
        ...
        if (this.user_info.withdraw_quota == 0) {
          ...
          const decimals = this.token_decimal > 8 ? 8 : this.token_decimal;
          this.withdraw_amount = parseFloat(
            fromWei(this.user_info.withdraw_quota, this.token_decimal)
          ).toFixed(decimals);
        }
      } else {
    ...
    async hasEnoughFToken(amount) {
      ...
      const balance = await this.market.contract.methods
        .balanceOf(account)
        .call();
